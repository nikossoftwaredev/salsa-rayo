...
export const generateInvoicePdf = async (data: InvoicePdfData): Promise<Buffer> => {
  ...
  if (data.qrUrl) {
    try {
      qrDataUri = await QRCode.toDataURL(data.qrUrl, { width: 140, margin: 1 })
    } catch {
      console.error("[Invoice PDF] Failed to generate QR code")
    }
  }
    ...
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.footer}>
          <View style={s.qrBox}>
            {qrDataUri ? (
              <Image src={qrDataUri} style={{ width: 60, height: 60 }} />
            ) : (
              <>
          ...
          <View style={{ alignItems: "flex-end" }}>
            ...
            <Text style={{ fontSize: 7, color: GRAY }}>{SITE_URL.replace("https://www.", "")}</Text>
          </View>
        </View>
      </Page>
    </Document>
