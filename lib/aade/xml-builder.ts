...
const buildCounterpartXml = (
  ...
  if (counterpart.address) {
    xml += `<address>`
    if (counterpart.address.street)
      xml += `<street>${escapeXml(counterpart.address.street)}</street>`
    if (counterpart.address.number)
      xml += `<number>${escapeXml(counterpart.address.number)}</number>`
    if (counterpart.address.postalCode)
      xml += `<postalCode>${escapeXml(counterpart.address.postalCode)}</postalCode>`
    if (counterpart.address.city)
      xml += `<city>${escapeXml(counterpart.address.city)}</city>`
    xml += `</address>`
  }
  xml += `</counterpart>`
  return xml
